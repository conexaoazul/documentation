...
This tab is visible for all employees, and does not require any other apps to be installed for this
to be visible.
...
- :guilabel:`Job Title`: This field is automatically populated with the selection made in the
  ...
  .. example::
     The typed-in description in the :guilabel:`Job Title` field can contain more specific
     information than the :guilabel:`Job Position`, if desired.

     For example, if someone is hired for a sales representative position configured as
     :guilabel:`Sales Associate` in the **Recruitment** app, that can be selected for the
     :guilabel:`Job Position` field.

     In the typed-in :guilabel:`Job Title` can be more specific, such as `Sales Associate -
     Subscriptions` if the employee is focused solely on subscription sales.
...
- :guilabel:`Levels`: Click :guilabel:`Add a line`, and enter a :guilabel:`Name` and
  ...
  .. example::
     To add a math skills in yellow, enter `Math` in the :guilabel:`Skill Type` field, and click the
...
.. note::
   This tab **only** appears if at least one :ref:`skill types <employees/skill-types>` is
   configured as *certification*. When adding certifications, **only** skill types marked as a
...
- :guilabel:`Bank Accounts`: Enter the bank account number using the drop-down menu. If the bank
  account does not exist, :ref:`cretae a new bank account <employees/add-bank>` and select it.
...
The employee's name populates the :guilabel:`Account Holder` and :guilabel:`Accoutn Holder Name`
...
.. important::
   **All** bank accounts must be marked as :guilabel:`Trusted`, if not payments cannot be processed
   and sent to the bank account. Having an :guilabel:`Untrusted` bank account for an employee will
   cause an error in the **Payroll** application.
...
- :guilabel:`SIM Card Copy`: Upload a copy of the SIM card if the employee issuing a work-issued
  mobile phone.
- :guilabel:`Internet Subscription Invoice`: If the employee is receiving benefits or compensation
  ...
  .. note::
     This is for documentation purposes only. Employees must use the **Expenses** app to request
     reimbursement for expenses, or define compensation in the *Payroll* tab.
...
- :guilabel:`Home-Work Distance`: Enter the distance from the employee's home to their work location
  in the first field. Set the second field to either kilometers (:guilabel:`km`) or miles
  (:guilabel:`mi`) using the drop-down menu. This information may be used for employee commuter
  benefits.
...
Alternatively, click :icon:`fa-envelope-o` :guilabel:`Invite teammates via email` that appears in
the :guilabel:`User` drop-down menu, and an *Invite teammmates* pop-up window loads, with the same
fields as the *Create User* pop-up window. Fill out the form, then click :guilabel:`Send
...
:guilabel:`Time Off` approver field does not appear. Only one selection can be made for each field.
  ...
  .. important::
     ...
     *Administrator* rights set for the corresponding human resources role.
     ...
     :guilabel:`Human Reosurces` and check the various settings.
