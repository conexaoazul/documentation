...
.. note::
   ...
   - You can switch between :doc:`multiple users <employee_login>`, provided :ref:`multi-employee
     management is enabled <pos/employee_login/use>` from an open POS register.
...
#. Click on products to add them to the cart.
   ...
   - To modify the product **price**, click :guilabel:`Price` and enter the new product price using
     the keypad.
#. Once an order is completed, click :guilabel:`Payment` to proceed to checkout, then:

   #. Select the :doc:`payment method <payment_methods>`.
   #. Enter the received amount.
   #. Click :guilabel:`Validate`.
#. Finally, click :guilabel:`New Order` to move on to the next order.
...
:guilabel:`Contact Info` category under the :guilabel:`Extra Info` tab, then click the
:icon:`fa-envelope` (:guilabel:`email`) icon or :icon:`fa-whatsapp` (:guilabel:`whatsapp`) icon.
...
#. On the payment screen, select a payment method, then click :guilabel:`Validate` to directly
   reimburse the customer. Or,

   #. Click :guilabel:`Back` to return to the POS register; a new order containing the returned
      items (marked with a negative quantity) is created automatically.
   #. Add a gift card to the order; its value automatically adjusts to match the total refund
      amount.
...
Notes allow you to attach extra information to specific products in an order. There are two types:
internal notes for staff and customer notes for the customer.
